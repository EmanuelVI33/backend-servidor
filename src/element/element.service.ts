...
export class ElementService {
  constructor(
    ...
    ffmpeg.setFfmpegPath(join(__dirname, '..', '..', 'src', 'ffmpeg', 'ffmpeg.exe')); // Ajusta la ruta según tu estructura de carpetas
  }
  ...
  async generateElement(elementTriggerDto: ElementsTriggerDto) {
    ...
    elementsWithTypes.map(async (element: Element) => {
      ...
      } else if (element instanceof Imagen) { //aqui
  ...
  async genImageByFile(image: Express.Multer.File, duration: number): Promise<string> {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const tempInputFilePath = path.join(__dirname, `input_${Date.now()}.jpg`);
